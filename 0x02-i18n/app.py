...
def get_timezone():
    """Returns timezone selection from the user settings or default"""
    try:
        # Find timezone parameter in URL parameters
        if request.args.get("timezone"):
            timezone: str = pytz.timezone(request.args.get("timezone"))
            return timezone
        # Find time zone from user settings
        if g.user:
            timezone: str = pytz.timezone(g.user.timezone)
            return timezone
