...
class MRUQueue:
    """Class to define MRU order Queue structure"""
    ...
    def remove(self) -> Any:
        """Removes the most recently used(front) item in the queue"""
        if not self.isEmpty():
            key = self.head.key
...
class MRUCache(BaseCaching):
    """Class to define MRU cache replacement algorithm"""

    def __init__(self, *args, **kwargs):
        """Instantiates new objects"""
        super().__init__(*args, **kwargs)
        self._queue = MRUQueue()
